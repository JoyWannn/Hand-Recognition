...
class HandGestureRecognizer:
    def __init__(self):
        ...
        self.KEY_COOLDOWN = 0.2  # seconds between key presses
    ...
    def recognize_gesture(self, landmarks):
        ...
        if all(fingers_extended.values()):
            ...
            #握拳
            return "CLOSED_FIST"
        elif fingers_extended['thumb'] and not any(v for k, v in fingers_extended.items() if k != 'thumb') and  thumb_tip.x < pinky_tip.x:
            #大拇指朝上(距離要再抓一下)
            return "right"
        elif fingers_extended['thumb'] and not any(v for k, v in fingers_extended.items() if k != 'thumb') and  thumb_tip.x > pinky_tip.x:
            #大拇指朝下(距離要再抓一下)
    ...
    def simulate_keyboard(self, gesture):
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                recognizer.simulate_keyboard(gesture)
